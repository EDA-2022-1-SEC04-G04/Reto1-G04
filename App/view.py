...
def printMenu():
    ...
    print("3 Listar albumes en un periodo deseado")
...
def printInfoAlbum(album):
    ...
    if size:
        ...
        for album in lt.iterator(album):
            print('Titulo del álbum: ' + album['name'] + '\nid ' + album['id'] + '\nFecha  publicacion: ' + album['release_date'])
...
def printAlbumsRange(rango):
    size = lt.size(rango)
    if size:
        print('Albumes en rango dado: ')
        for track in lt.iterator(rango):
            print('Albumes: ')
...
while True:
    ...
    if int(inputs[0]) == 1:
        ...
        fechaI = input("fecha de inicio: ")
        fechaF = input("fecha final: ")
        rangoFechas = controller.getAlbumsRange(control, fechaI, fechaF)
        printAlbumsRange(rangoFechas)
        print("gracias wuapo :3")
